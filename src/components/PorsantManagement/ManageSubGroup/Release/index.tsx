...
const Release = () => {
  ...
  const handleUploadFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allowedTypes = [
      'image/png', 'image/jpg', 'image/jpeg', // تصاویر
      'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', // اکسل
      'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', // ورد
      'application/vnd.ms-access', // اکسس
      'text/csv', // CSV
      'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', // پاورپوینت
      'application/pdf', // پی‌دی‌اف
      'text/html', 'application/xhtml+xml' // HTML
    ]
  
    const newUploadStatuses: Record<number, { status: 'idle' | 'uploading' | 'success' | 'error', progress: number }> = {}
  
    for (const file of files) {
      const fileId = file.lastModified // استفاده از lastModified به‌عنوان ID
  
      if (!allowedTypes.includes(file.type)) {
        showErrorModal('پسوند فایل قابل قبول نمی‌باشد')
        newUploadStatuses[fileId] = { status: 'error', progress: 0 }
        continue
      }
  
      if (file.size < 50000) {
        showErrorModal('حجم فایل کمتر از حد مجاز است')
        newUploadStatuses[fileId] = { status: 'error', progress: 0 }
        continue
      }
  
      if (file.size > 2200000) {
        showErrorModal('حجم فایل بیشتر از حد مجاز است')
        newUploadStatuses[fileId] = { status: 'error', progress: 0 }
        continue
      }
  
      newUploadStatuses[fileId] = { status: 'uploading', progress: 0 }
      ...
      try {
        ...
        reader.onloadend = async () => {
          try {
            ...
            const accessToken = (await getCookieByKey('access_token')) || ''
            await AddDocFile({ src: formData, accessToken })
            ...
            showErrorModal('خطا در بارگذاری')
          }
        }
        reader.readAsDataURL(file)
      } catch (error) {
        ...
        showErrorModal('خطا در پردازش فایل')
      }
    }
  }
                      ...
                      {uploadStatuses[index]?.status === 'uploading' && (
                        <div className='w-full bg-gray-200 rounded-full h-2 mt-2'>
                          <div
                            className='bg-[#7747C0] h-2 rounded-full transition-all duration-500'
                            style={{
                              width: `${uploadStatuses[index]?.progress}%`,
                            }}></div>
                        </div>
                      )}
                    </label>
